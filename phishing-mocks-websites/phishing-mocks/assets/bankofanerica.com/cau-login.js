function getSCookie(listOfCookies) {
  var  cookieSet = [];
// The below logic is to determine if the value is single or multiple values
if(!Array.isArray(listOfCookies)){
   cookieSet.push(listOfCookies);
}else{
cookieSet = listOfCookies;
}
for(i=0; i<cookieSet.length; i++){
var re = new RegExp('[; ]' + cookieSet[i] + '=([^\\s;]*)')
   , matches = null;
